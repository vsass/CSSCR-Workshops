...
::: fragment
```{r workflow, fig.align = 'center', out.width = "50%", fig.cap = "What happens when we render an .Rmd file?"}
...
-   The `rmarkdown` package expanded what can be done with these types of documents, which now includes:
...
- Ready-made templates with the [`prettydocs`](https://prettydoc.statr.me/) package
  - See more examples [here](https://www.datadreaming.org/post/r-markdown-theme-gallery/)
...
```` ```{r pressure, echo = FALSE, fig.height = 4, fig.width = 4, fig.cap = "Pressure plot"} ````
